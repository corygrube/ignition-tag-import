...
udt_raw = [
	...
	['XmtrRedundant',	'Standard/XmtrRedundant/gtypXmtrRedundant',	'gtypXmtrRedundant',	None,				'gt_HmiData_'],
	['Sequence',		'Standard/Sequence/gtypSequence',			'gtypSequenceHmiData',	None,				'gt_SequenceHmiData_'],
	['Totalizer',		'Standard/Totalizer/gtypTotalizer',			'gtypTotalizer',		None,				'gt_HmiData_'],
	['Module_Status',	'Custom/Module_Status/Module_Status',		'Module_Status',		None,				'gt_HmiData_']
]
...
workbook_path = 'tags.csv'
temp_workbook_path = 'temp/temp_workbook.xlsx'
...
for udt_name in udt_dict:
	...
	for row in range(sheet.nrows):
		...
		if sheet.cell_value(row,0) == 'TAG' and sheet.cell_value(row,1) == '' and sheet.cell_value(row,4).lower() == udt_dict[udt_name]['udt_plc'].lower():
		...
		# Aliased UDT build (if alias defined)
			...
			if sheet.cell_value(row,0) == 'ALIAS' and str(sheet.cell_value(row,5)).lower().startswith(udt_dict[udt_name]['udt_plc_alias'].lower()):
				temp_name = sheet.cell_value(row,2)
				temp_name = re.sub(udt_dict[udt_name]['udt_plc_prefix'], '', temp_name, flags=re.I)
				temp_udt = tag_udt(temp_name, udt_name, plc)
				temp_udt.documentation = sheet.cell_value(row,3)
...
output_path = 'output/' + plc + ' tag import ' + datetime_string + '.json'
